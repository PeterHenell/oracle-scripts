...
IS
      ...
      OPEN retval FOR
         SELECT *
          ...
          WHERE exists ( SELECT 'x'
						 ...
						 where column_value = in_clause_tab.id));
   ...
   IS
      ...
      PROCEDURE test_nested_table_list3
      IS
         l_list pky_nt := pky_nt ( 1, 3 );
         l_cv sys_refcursor;
         l_one in_clause_tab%ROWTYPE;
      BEGIN
         init_test ( 'Output from NESTED_TABLE_LIST w/o CAST' );

         FOR indx IN 1 .. iterations_in
         LOOP
            l_cv := nested_table_list3 ( l_list );
