CREATE OR REPLACE PROCEDURE show_all_arguments (
   program_in                IN   VARCHAR2
 , show_collections_in       IN   BOOLEAN DEFAULT TRUE
 , show_loading_process_in   IN   BOOLEAN DEFAULT FALSE
)
IS
   dblink           VARCHAR2 (100);
   part1_type       NUMBER;
   object_number    NUMBER;
   l_all_argument   all_arguments%ROWTYPE;
   ...
   BEGIN
      ...
      THEN
         ...
         DBMS_OUTPUT.put_line
            ('Object               OvLd Pos Lev Seq Type            Name                               Mode  '
            );
         DBMS_OUTPUT.put_line
            ('-------------------- ---- --- --- --- --------------- --------------------------------- ------ '
            );
         ...
         LOOP
            DBMS_OUTPUT.put_line
                            (   RPAD (l_arguments (argrow).object_name, 20)
                             || strval (l_arguments (argrow).overload, 4)
                             || strval (l_arguments (argrow).POSITION, 4)
                             || strval (l_arguments (argrow).SEQUENCE, 4)
                             || strval (l_arguments (argrow).data_level, 3)
                             || RPAD (l_arguments (argrow).data_type, 15)
   ...
   IS
      ...
      FUNCTION is_function (prog_in IN VARCHAR2, ovld_in IN PLS_INTEGER)
         RETURN VARCHAR2
      IS
      BEGIN
   ...
   BEGIN
      ...
      THEN
         ...
         DBMS_OUTPUT.put_line ('');
         DBMS_OUTPUT.put_line
                       ('Information about distinct overloadings of programs:');
         DBMS_OUTPUT.put_line ('');
         ...
         LOOP
            ...
            LOOP
               DBMS_OUTPUT.put_line
                            (   LPAD (l_program || '-' || indx, 20)
                             || ' '
                             || LPAD (is_function (l_program, indx), 12)
                             || ' '
                             || LPAD
                                   (TO_CHAR (l_programs (l_program) (indx).COUNT
                                            )
                                  , 6
                                   )
                            );
   ...
   PROCEDURE show_loading (rec_in IN all_arguments%ROWTYPE)
   IS
   BEGIN
      DBMS_OUTPUT.put_line (   'Loading - program "'
                            || rec_in.object_name
                            || '" overload '
                            || NVL (rec_in.overload, 0)
                            || ' position '
                            || rec_in.POSITION
                            || ' data level '
                            || rec_in.data_level
                           );
      DBMS_OUTPUT.put_line (   '   Count in programs collection = '
                            || l_programs.COUNT
                           );
      DBMS_OUTPUT.put_line (   '   Count in overloads for "'
                            || rec_in.object_name
                            || '"-'
                            || NVL (rec_in.overload, 0)
                            || ' = '
                            || l_programs (rec_in.object_name).COUNT
                           );
      DBMS_OUTPUT.put_line
                   (   '   Count in arguments for overloading "'
                    || rec_in.object_name
                    || '"-'
                    || NVL (rec_in.overload, 0)
                    || ' = '
                    || l_programs (rec_in.object_name) (NVL (rec_in.overload
                                                           , 0
                                                            )
                                                       ).COUNT
                   );
...
BEGIN
   ...
   DBMS_OUTPUT.put_line (   'Show argument information for '
                         || l_owner
                         || '.'
                         || NVL (l_package_name, l_object_name)
                        );
   ...
   LOOP
      ...
      l_programs (rec.object_name) (NVL (rec.overload, 0)) (rec.POSITION)
                                                              (rec.data_level) :=
                                                                          rec;

      IF show_loading_process_in
      THEN
         show_loading (rec);
      END IF;
   END LOOP;

   IF show_collections_in
   THEN
