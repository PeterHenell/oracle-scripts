...
IS
   FUNCTION only_identifier_from (string_in     IN VARCHAR2,
   ...
   FUNCTION all_in_string (string_in      IN VARCHAR2,
   ...
   FUNCTION distinct_placeholders (list_in IN placeholder_aat)
      RETURN INTEGER
   IS
      TYPE used_t IS TABLE OF BOOLEAN
   ...
   BEGIN
      ...
      LOOP
         IF NOT l_used.EXISTS (list_in (indx).name)
         THEN
            l_used (list_in (indx).name) := TRUE;
         END IF;
      END LOOP;
   ...
   FUNCTION count_in_string (string_in      IN VARCHAR2,
                             dyn_plsql_in   IN BOOLEAN DEFAULT FALSE)
      RETURN PLS_INTEGER
   IS
      l_placeholders   placeholder_aat;
   ...
   FUNCTION nth_in_string (string_in      IN VARCHAR2,
   ...
   PROCEDURE show_placeholders (list_in        IN placeholder_aat,
                                dyn_plsql_in   IN BOOLEAN DEFAULT FALSE)
   IS
   ...
   BEGIN
      ...
      LOOP
         DBMS_OUTPUT.put_line (
            list_in (l_index).name || ' - ' || list_in (l_index).position);
         l_index := list_in.NEXT (l_index);
      END LOOP;
   END show_placeholders;

   PROCEDURE show_placeholders (string_in      IN VARCHAR2,
                                dyn_plsql_in   IN BOOLEAN DEFAULT FALSE)
   IS
   BEGIN
   ...
   FUNCTION eq (rec1_in       IN placeholder_rt,
