...
/* Cannot reference PL/SQL types */

CREATE OR REPLACE PROCEDURE updnumval (rec_in             IN employees_m2%ROWTYPE
                                     , department_id_in   IN PLS_INTEGER
                                     , val_in             IN NUMBER)
IS
   l_update   VARCHAR2 (1000)
      := 'UPDATE employees_m2 SET ROW = :rowdata
              WHERE department_id = :dept and :val IS NOT NULL';
BEGIN
