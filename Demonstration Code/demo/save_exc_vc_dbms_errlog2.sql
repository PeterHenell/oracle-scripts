...
alter table t_errors add username varchar2(100)
/

CREATE OR REPLACE TRIGGER t_errors_log_user
   BEFORE INSERT
   ON t_errors
   FOR EACH ROW
BEGIN
...
BEGIN
   DBMS_OUTPUT.put_line (   'Error message after DELETE: '
...
SELECT DISTINCT ora_err_mesg$, username
           FROM t_errors
/