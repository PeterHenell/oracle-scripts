DECLARE
   ...
   TYPE employment_tt IS VARRAY(4) OF employment_rt;

   -- Must initialize a VARRAY.
   ends_not_meeting   employment_tt := employment_tt();
   
   PROCEDURE addone (description_in IN VARCHAR2, population_in IN PLS_INTEGER)
   IS
   BEGIN
      -- Must explicitly extend a VARRAY before adding data.
...
BEGIN
   ...
   -- Always "safe" to use a FOR loop with VARRAYs.
