...
/* Bulk collect of ref cursor */
CREATE OR REPLACE FUNCTION stockpivot_nopl_bulk (dataset refcur_pkg.refcur_t)
   RETURN tickertypeset
IS
   out_obj    tickertype        := tickertype (NULL, NULL, NULL, NULL);
   --
...
IS
   ...
   PROCEDURE transformative_nopl_bulk
   IS
   BEGIN
      init;
      ...
      INSERT INTO tickertable
         SELECT *
           FROM TABLE (stockpivot_nopl_bulk (CURSOR (SELECT *
                                                  FROM stocktable)));
   ...
   BEGIN
      ...
      LOOP
         ...
         INSERT INTO tickertable
              ...
              VALUES (mystock (indx).ticker, mystock (indx).pricedate
                     ,mystock (indx).pricetype, mystock (indx).price
                     );
   ...
   BEGIN
      ...
      LOOP
         INSERT INTO tickertable
              ...
              VALUES (rec.ticker, rec.trade_date, 'O', rec.open_price
                     );

         INSERT INTO tickertable
                     (ticker, pricedate, pricetype, price
                     )
              VALUES (rec.ticker, rec.trade_date, 'C', rec.close_price
                     );
      END LOOP;
