...
CREATE OR REPLACE PROCEDURE pl (
   str IN VARCHAR2, len IN INTEGER := 80, expand_in in BOOLEAN := TRUE)
   /* Last Chicago seminar 5/2000 */
IS
...
AS
   v_count INTEGER;
BEGIN
   select count(*) into v_count from stolen_life;
   pl ('');
   pl ('AUTHID DEFINER STOLEN_LIFE COUNT: ' || v_count);
...
BEGIN
   ...
   select count(*) into v_count from stolen_life;
   pl ('');
   pl ('AUTHID CURRENT_USER STOLEN_LIFE COUNT: ' || v_count);
