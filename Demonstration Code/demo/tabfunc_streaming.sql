...
CREATE OR REPLACE FUNCTION tickerpivot (dataset refcur_pkg.refcur_t)
   RETURN stocktypeset
IS
   out_obj     stocktype := stocktype (NULL, NULL, NULL, NULL);
...
BEGIN
   LOOP
      ...
      LOOP
         ...
         THEN
            out_obj.openprice := l_dataset (l_row).price;
            out_obj.closeprice := NULL;
         ELSE
            out_obj.openprice := NULL;
            out_obj.closeprice := l_dataset (l_row).price;
         END IF;
